...
def get_rotation_matrix_from_6d_vector(
    ...
    """
    v1 = rot_vector[:, 0:3] # N x 3
    v2 = rot_vector[:, 3:6] # N x 3
    r1 = v1 / torch.linalg.norm(v1, dim=1).unsqueeze(1) # N x 3
    r2 = v2 - (v2 * r1).sum(1).unsqueeze(1) * r1 # N x 3
    r2 = r2 / torch.linalg.norm(r2, dim=1).unsqueeze(1) # N x 3
    r3 = torch.cross(r1, r2) # N x 3
